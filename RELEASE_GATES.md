...
| LinkedIn recommendation provenance and publication | Carl + content | blocked | Official LinkedIn export reconciliation, exact-text and attribution review, publication approval, and takedown path for every public record | Complete `PORT-REC-001`; candidate fixtures are not publication evidence |
...
| Public-approved career evidence | Carl + Jolene evidence reviewer | blocked | A versioned, reviewed, content-minimized export containing only active `public_approved` evidence, with manifest version/hash and revocation behavior verified | `JOL-CAREER-004` must deliver the deny-by-default public export; private registry data is not release evidence |
| Public Jolene delegate availability | Jolene engineering | blocked | A deployed, isolated public service that implements the frozen contract and consumes only the approved export | `JOL-CAREER-005` must pass its own security and operational checks |
| Portfolio live-Jolene integration | Portfolio engineering | blocked | Adapter compatibility tests, public-service integration, safe degraded mode, and verified absence of private API, Obsidian, MCP, and durable-memory access | Requires completed `JOL-CAREER-004`, `JOL-CAREER-005`, `PORT-CONTRACT-001`, and `PORT-JOL-009` |
| Jolene security and privacy | Portfolio + Jolene engineering | blocked | Input and output limits, rate limits, cost ceilings, redaction, abuse controls, kill switch, minimized retention, and privacy review | Complete `PORT-JOL-008`; the public service and export must exist before final verification |
| Jolene grounding and misuse evaluation | Portfolio + Jolene evaluation owners | blocked | Passing grounding, citation, job-fit classification, prompt-injection, privacy, abuse, and cost evaluations with explicit release thresholds | Complete `JOL-CAREER-006` and `PORT-EVAL-001` after the live public boundary exists |
| Contact-intent handoff | Carl + portfolio engineering | not started | Consent copy, minimized payload, review-only handoff, retention behavior, abuse controls, and proof that no autonomous outbound action is possible | Complete `PORT-JOL-007`; otherwise omit the contact-intent capability from release |
...
| Analytics and visitor privacy | Carl + portfolio engineering | not started | Approved event taxonomy, data minimization, consent behavior where required, retention limits, provider configuration, and proof that sensitive chat or job-description content is excluded | Complete `PORT-ANALYTICS-001`; otherwise ship with analytics disabled |
| Security and container boundary | Portfolio engineering | in progress | Green container build/scan, non-root read-only runtime, secret-boundary checks, dependency review, and public-surface threat review | CI evidence exists for the current code, but must pass again on the final release commit |
