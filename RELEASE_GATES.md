...
| Public-approved career evidence | Carl + Jolene evidence reviewer | blocked | A versioned, reviewed, content-minimized export containing only active `public_approved` evidence, with manifest version/hash and revocation behavior verified | `JOL-CAREER-004` delivered the deny-by-default export and a valid empty corpus; Carl must approve eligible career claims, then the artifact must be regenerated and verified |
| Public Jolene delegate availability | Jolene engineering | blocked | A deployed, isolated public service that implements the frozen contract and consumes only the approved export | Local manifest, answer, job-fit, admission, and kill-switch slices exist, but parent ticket `JOL-CAREER-005` remains incomplete; no public endpoint or deployment is live |
