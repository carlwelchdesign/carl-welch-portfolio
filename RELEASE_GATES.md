...
| Production origin and professional share previews | Carl + engineering | passed | Approved production origin plus verified canonical URLs, robots, sitemap, favicon, Open Graph/X metadata, social images, and provider previews | Sites version 3 is live at `https://carl-welch-portfolio.flakeysaturation.chatgpt.site`; exact-origin validation covers 11 routes and every distinct share image |
...
| Performance and resilience | Portfolio engineering | passed | Production-origin Core Web Vitals/performance results, animation and diagram budgets, degraded-state behavior, and representative mobile traces | Controlled public-origin measurements pass at desktop, mobile, and throttled 4G plus 4× CPU; field data will accumulate after launch |
...
| Security and container boundary | Portfolio engineering | passed | Green container build/scan, non-root read-only runtime, secret-boundary checks, dependency review, and public-surface threat review | Current release commit `234ac70437491a21aa495ad5a92cf633a87774ac` passed browser regression plus hardened image build, HIGH/CRITICAL scan, health, and runtime-boundary checks in run `33042009323` |
| Deployment configuration | Carl + portfolio engineering | passed | Carl's explicit deployment approval, reviewed hosted configuration, production environment values, and successful release-candidate deployment | Carl authorized completion; Sites version 3 from release commit `234ac70437491a21aa495ad5a92cf633a87774ac` is live and exact-origin smoke verification passes |
| Rollback and kill-switch rehearsal | Portfolio engineering + Jolene engineering | passed | Tested portfolio rollback, public-Jolene kill switch, evidence revocation propagation, owner runbook, and recovery verification | Static version 3 was rolled back to version 2 and restored; public Jolene remains disabled, and the operations runbook records the recovery path |
