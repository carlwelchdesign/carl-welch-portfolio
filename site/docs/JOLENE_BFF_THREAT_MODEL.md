...
- The future upstream may only be the isolated public Jolene service. Its configured origin must be a public HTTPS origin. Fixed v1 paths prevent visitor-controlled SSRF destinations.
...
| Credential theft / SSRF | Server-only bearer credentials, fixed endpoint paths, HTTPS public-origin validation, redirects disabled. | Rotate credentials and restrict the credential to the public service only. Enforce outbound allowlisting at the deployment layer. |
...
The code-level controls are testable before deployment, but live activation remains blocked on `PORT-JOL-009`, the public-service deployment dependency, distributed rate/cost enforcement, edge/WAF configuration, secrets provisioning and rotation, monitoring/alerts, retention/deletion approval, and a kill-switch rehearsal. Green local or container checks do not close those operational gates.
